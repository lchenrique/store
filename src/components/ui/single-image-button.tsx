...
import { CircleUserRound, X , Image as LucidImage} from "lucide-react";
import Image from "next/image";
import React, { useCallback, useEffect, useRef, useState } from "react";

export interface SingleImageButtonProps {
  onImageChange: (file: File | null) => void;
  imageUrl: string | null;
}

export default function SingleImageButton({ onImageChange, imageUrl }: SingleImageButtonProps) {
        ...
        >
          {previewUrl ? (
            ...
            <div aria-hidden="true">
              <LucidImage  className="opacity-60" size={16} strokeWidth={2}  />
            </div>
          )}
        </Button>
        {previewUrl && (
